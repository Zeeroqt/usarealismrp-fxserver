...
local carspawns = {
	[1] =  { ['coords'] = vector4(-232.9496, 308.771, 91.79194, 270.2435), ['info'] = ' 1' }
}

local pillWorker = { ['coords'] = vector4(-235.6822, 311.3096, 92.28311, 274.9692), ['info'] = ' store ' }

local peds = {
	{x = -235.6822, y = 311.3096, z = 91.28311, hash = 846439045, heading = 274.9692}
}
...
function CreateOxyVehicle()
	...
	local vehicle_key = {
        name = "Key -- " .. GetVehicleNumberPlateText(oxyVehicle),
        quantity = 1,
        type = "key",
        owner = "Unknown",
        make = "Stolen",
        model = "Uknown",
        plate = GetVehicleNumberPlateText(oxyVehicle)
    }
    -- give key to owner
...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			exports.globals:notify('The delivery was on point, your GPS will be updated with the next drop off', 'success')
		else
			exports.globals:notify('The drop-off failed', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			exports.globals:notify('You are close to the drop-off point')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			exports.globals:notify('You took too long', 'error')
		end
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			DrawText3Ds(pillWorker['coords']["x"],pillWorker['coords']["y"],pillWorker['coords']["z"], "~g~[E]~s~ $2000 - Start Oxy Run") 
	...
	while true do
		...
		if OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				exports.globals:notify("The dealer isn't giving you anymore locations due to the state of his car", "error")
			else
				if tasking then
					Citizen.Wait(30000)
				else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	exports.globals:notify('A car has been provided. Your GPS will be updated with locations soon', 'success')
end)
